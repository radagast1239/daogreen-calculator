...
(function(global){
  ...
  var UIL = {
    ru: {
      ...
      'ui.mass.vfHint': 'свернуть → стандарт VF',
      'ui.grow.vfHint': 'свернуть → подставить стандарты VF',
      ...
      'cvCompare.intro': 'Выберите несколько сортов при одинаковых настройках (свет, температура, плотность, срок). Таблица и график обновляются сразу. Выделенная колонка — текущий сорт.',
      ...
      'ui.vf.stdLocksVf': 'Подставлять из справочника VF:',
    ...
    en: {
      ...
      'ui.mass.vfHint': 'collapse → VF standard',
      'ui.grow.vfHint': 'collapse → apply VF standards',
      ...
      'cvCompare.intro': 'Pick several cultivars under the same settings (light, temperature, density, timing). The table and chart update instantly. Highlighted column — active cultivar.',
      ...
      'ui.vf.stdLocksVf': 'Use VF catalog for:',
  ...
  function applyUiI18n(){
    ...
    document.querySelectorAll('#block-grow-time .collapse-vf-only').forEach(function(el){ el.textContent = uiT('ui.grow.vfHint'); });
    document.querySelectorAll('#block-mass .collapse-vf-only').forEach(function(el){ el.textContent = uiT('ui.mass.vfHint'); });
